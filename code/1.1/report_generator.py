...
def generate_pdf_report(path, report_data, plot_data_list):
    ...
    # 4. 主频率数据表格 (全时段)
    story.append(Paragraph("Top Frequencies (Overall)", styles['Heading2']))
    story.append(Spacer(1, 0.1 * inch))

    table_data = report_data.get('main_frequency_data', [])
    if not table_data: table_data = [["Rank", "Frequency (MHz)", "Count"]]
    ...
    # 渲染自定义时间范围的表格
    ranged_table_data = report_data.get('ranged_frequency_data')
    if ranged_table_data and len(ranged_table_data) > 1:  # 确保有数据行
        story.append(Paragraph("Top Frequencies (Custom Time-Range)", styles['Heading2']))
        story.append(Spacer(1, 0.1 * inch))

        ranged_table = Table(ranged_table_data)
        # 可以复用上面的样式，或者为它创建新样式
        ranged_table.setStyle(table_style)
        story.append(ranged_table)
        story.append(Spacer(1, 0.2 * inch))
    ...
    if plot_data_list:
        # 使用PageBreak()可以确保第一个图表后换页
        #story.append(PageBreak())

        for plot_info in plot_data_list:
            plot_title = plot_info.get('title', 'Analysis Chart')
